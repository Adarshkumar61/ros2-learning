# #this code Changes TIMER SPEED using PARAMETER
...
class myparamnodee(Node):
    def __init__(self):
        super().__init__('myparamnodee')

        self.declare_parameter('robot_speed', 1.0)
        
        self.speed = self.get_parameter('robot_speed').value

        self.timer = self.create_timer(self.speed, self.callback)
    ...
    def param_callback(self, params):
        for param in params:
            if param.name == 'robot_speed':
                if param.value < 0:
                    self.get_logger().info('Robot speed cant go -ve or zero')
